...
traps_paired_no_zeros %>%
  ...
  theme_hc() +
  theme(axis.text.x = element_text(angle=45, hjust = 0.7), axis.text = element_text(size = 20), axis.title = element_text(size = 26), legend.position = c(0.8, 0.7), legend.text = element_text(size = 20)) +
  scale_x_discrete(limits = fish_grouping) +
  geom_text( aes(y=prop_fish+2, label = ifelse(p_value<0.05 & Exp_or_Cont != "Control","*","")), size = 14) +
  annotate("rect", xmin = 0, xmax = 5.5, ymin = 0, ymax = 40, alpha = .2) +
  annotate("text", label = "Thin-bodied fish", x = 4, y = 35, alpha = 0.6, size = 8) +
  annotate("text", label = "Thick-bodied fish", x= 10, y = 35, size = 8)
