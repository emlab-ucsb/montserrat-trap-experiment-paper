...
```{r length-frequency histogram and Kolmogorov-Smirnov test}
...
plot_grid(species_hist_plots, legend, rel_widths = c(2, .3))
...
```{r l-f soak time}
soak_time_lengt_freq_allplots <- traps_paired_no_zeros %>% 
  filter(Length_cm < 50) %>% 
...
cowplot::plot_grid(combined_plot, legend, rel_widths = c(1,0.1)) +
  draw_label("Number of fish", x= 0, y=0.5, vjust= 1.5, angle=90)
