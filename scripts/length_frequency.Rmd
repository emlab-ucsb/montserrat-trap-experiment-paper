...
gt(mean_length_Mat50_table) %>% 
  cols_label(species = "Species name",
             Mat50 = "Lmat (cm)",
             Control = "Mean length in control traps (cm)",
             Experimental = "Mean length in experimental traps (cm)",
             `Mean length greater than Mat50?` = "Mean length greater than Lmat?") %>% 
  gtsave("mean_length_Lmat_table.rtf", path = "../outputs/")

```
## Proportions of fish below and above Lmat
```{r proportions of fish below and above Lmat}
traps_paired_no_zeros %>% 
  select(Exp_or_Cont, common, sciname, Length_cm) %>% 
  inner_join(lmat_values, by = "common") %>% 
  filter(!is.na(Mat50)) %>% 
  mutate(Length_greater_Lmat = case_when(Length_cm>Mat50 ~ 1,
                                         TRUE ~ as.numeric(0))) %>% 
  group_by(Exp_or_Cont, common, sciname) %>% 
  summarise(Mat50 = mean(Mat50),
            mean_length = mean(Length_cm),
            perc_greater_Lmat = sum(Length_greater_Lmat)/n()) %>% 
  arrange(common, Exp_or_Cont)
